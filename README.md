<p align = "center">
    <img src="frontend/public/ghost-icon-lg.png" alt="Icon" style="width:100px; height:100px;"> 
</p>
...
```
Set `POSTGRES_PASSWORD` in `.env`, and use the **same** value in `backend/.env.production`'s `DATABASE_URL`. Set `SECRET_KEY` in `backend/.env.production` with a separate, independently-generated value.
...
### Account management
Logged-in users can update their display name and password via an in-app modal (`PUT /api/auth/me`). Changing the password requires re-entering the current one. Password strength rules (minimum 8 characters, upper/lowercase, digit) are enforced on both registration and password changes via a shared backend validator.

### In-app notifications
The frontend surfacces upcoming assessment due dates and events as a ppersistent bell-icon dropdown, based on a 24-hour reminder threshold. This is computed entirely client-side from existing data.
...
This project includes a separate, production-oriented Docker Compose configuration (`docker-compose.prod.yml`), used by `./start.sh`/`./stop.sh` for local use. It differs from the development setup in several deliberate ways:

### Key differences
- Frontend is compiled to static files at build time (`npm run build`) rather than run through Vite's dev server; no hot-reload, no bind-mounted source.
...
- The production database uses a separate named volume (`db_data_prod`) from development's `db_data`, so the two environments never share data.
...
The current configuration runs over plain HTTP, suitable for local use only. A real public deployment would additionally need: HTTPS, `CORS_ORIGINS` pointed at a real domain, and a hosting target.
