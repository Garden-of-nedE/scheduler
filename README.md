...
- For production: replace the plaintext `.env` with a real secrets manager, put HTTPS in front of both services, and remove the `--reload` flag/bind-mounted volumes from `docker-compose.yml`